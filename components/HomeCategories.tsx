...
const HomeCategories = ({ categories }: { categories: Category[] }) => {
  return (
    <div className="bg-darkColor border border-shop_light_green/20 my-10 md:my-20 p-5 lg:p-7 ">
      <Title className="border-b border-shop_light_green/20 ">Popular Categories</Title>
      <div className="mt-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-5">
        {categories?.map((category) => (
          <div
            key={category?._id}
            className="bg-black/30 text-gray-300 p-5 flex items-center gap-3 group"
          >
            {category?.image && (
